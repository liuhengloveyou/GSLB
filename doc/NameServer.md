...
| data | 解析结果数组                                                 |
| n    | 请求解析的域名。                                             |
| ttl  | 该域名解析结果的TTL缓存时间。 用户应该按这个TTL时间对域名解析结果进行缓存。在TTL过期之前，直接使用缓存的IP；TTL过期后，再去请求HTTPDNS服务获得最新的解析结果。 |
| rs   | 该域名的解析结果列表，包括解析结果记录以及记录类型(rtype)：A、CNAME等。和这个地址上(A记录)提供服务的协议及端口号；协议包括：http、http2中的一个或多个。 |
...
取客户端ip，查询IP段得到客户端所在的线路和区域，跟据线路和区域就近调度。配置层级是 `/线路/区域`

**线路** 国内用户区分到电信、联通、移动…等运营商。国外区分到国家。

**区域** 只在国内用户区分，粒度到市。可配置单个客户端IP为独立的解析区域。
...
### 4. 默认解析
 ...
 每个域名的每个线路都有一个默认解析值，当没有更精细的解析命中，返回默认值。配置时以`*`表示。
